...
<script>
            ...
            while(div.firstChild){
              ...
              loadhtml = "<table>";
                ...
                for(var idx = 0; idx < idarray.length; idx++)
                {
                    ...
                    groupData["teams"] = [];
                    groupData["matches"] = [
                    { "id": 1,  "round": 0, "a": { "team": 0, "score": 0 }, "b": { "team": 0, "score": 0 } },
                    { "id": 2,  "round": 0, "a": { "team": 0, "score": 0 }, "b": { "team": 0, "score": 0 } },
                    { "id": 3,  "round": 0, "a": { "team": 0, "score": 0 }, "b": { "team": 0, "score": 0 } }  ];
        ...
        if (typeof(Storage) !== "undefined") {
          if (localStorage.top) {
            ...
            if( i_top != -1 ) {
              ...
              recode.push({'name':top, 'score':1});
            }
            recode.sort(compare);

            localStorage.top = JSON.stringify(recode);
          } else {
            localStorage.top = JSON.stringify([{'name':top, 'score':1}]); 
          }
          showResult();
        } else {
          // document.getElementById("result").innerHTML = "Sorry, your browser does not support web storage...";
    ...
    $("button#tournament").click(function(event){
        ...
        function RandomShuffle(o) {
          ...
          var len = Math.pow(2, Math.ceil(Math.log(o.length) / Math.log(2))-1), _i;
      	  var len_team = 2 * len;
